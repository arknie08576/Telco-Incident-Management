...
public class IncidentUpdateRequest {
    ...
    @Pattern(regexp = ".*\\S.*", message = "region must not be blank")
    @Size(max = 100, message = "region must not exceed 100 characters")
    private String region;

    @Schema(description = "Source alarm type reported by monitoring.", example = "POWER")
    @Pattern(regexp = ".*\\S.*", message = "sourceAlarmType must not be blank")
    ...
    @Schema(description = "Optional replacement for the full incident node set, including exactly one ROOT node.")
    @Valid
