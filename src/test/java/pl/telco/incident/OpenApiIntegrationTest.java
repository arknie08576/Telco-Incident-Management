...
class OpenApiIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    void apiDocsShouldExposeIncidentEndpointsAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/network-nodes/{id}'].get.summary").value("Get network node by ID"))
                ...
                .andExpect(jsonPath("$.paths['/api/maintenance-windows/{id}'].get.summary").value("Get maintenance window by ID"))
                ...
                .andExpect(jsonPath("$.paths['/api/alarm-events/{id}'].get.summary").value("Get alarm event by ID"))
                .andExpect(jsonPath("$.paths['/api/alarm-events/{id}'].patch.summary").value("Update alarm event"))
                ...
                .andExpect(jsonPath("$.paths['/api/maintenance-windows'].get.parameters[*].name", hasItem("statuses")))
                .andExpect(jsonPath("$.paths['/api/maintenance-windows'].get.parameters[*].name", hasItem("nodeId")))
                .andExpect(jsonPath("$.paths['/api/alarm-events'].get.parameters[*].name", hasItem("severities")))
                .andExpect(jsonPath("$.paths['/api/alarm-events'].get.parameters[*].name", hasItem("incidentId")))
                ...
                .andExpect(jsonPath("$.components.schemas.AlarmEventUpdateRequest.properties.incidentId.type").value("integer"))
                .andExpect(jsonPath("$.components.schemas.MaintenanceWindowPageResponse.properties.content.type").value("array"))
                .andExpect(jsonPath("$.components.schemas.AlarmEventPageResponse.properties.content.type").value("array"))
